...
- Pydantic как сериализатор и валидатор запросов в БД
- Opensensus как инструмент для телеметрии и трассировки
- Celery как реализация обработчика ассинхронных задач и работой с очередью сообщений (В процессе реализации)
