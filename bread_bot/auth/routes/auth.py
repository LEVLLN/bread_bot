...
from bread_bot.utils.dependencies import OffsetQueryParams, get_async_session
...
async def create_user(user_schema: UserCreateSchema, db: AsyncSession = Depends(get_async_session)):
    """
    Зарегистрировать пользователя
    """
    user_schema.hashed_password = get_password_hash(user_schema.hashed_password)
    try:
...
@router.get("/users", response_model=list[UserInfoSchema], dependencies=[Depends(get_current_active_admin_user)])
async def get_users(db: AsyncSession = Depends(get_async_session), offset_params: OffsetQueryParams = Depends()):
    """
