...
from bread_bot.common.models import Member
from bread_bot.common.clients.telegram_client import TelegramClient
from bread_bot.utils.testing_tools import test_app, TEST_SERVER_URL, init_async_session
...
class GetMessageTestCase(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_root_url(self):
        respx.post("https://api.telegram.org/bot/sendMessage").mock(
        ...
        member: Member = await Member.async_first(db=self.session, where=Member.username == "Lev_key")
        self.assertEqual(member.username, "Lev_key")
        self.assertEqual(member.first_name, "Lev")
        self.assertEqual(member.last_name, "Kudryashov")
        self.assertFalse(
            member.is_bot,
        )
    ...
    async def test_not_valid_body(self):
        ...
        self.assertEqual(
            response.json(),
            {"detail": [{"loc": ["body", "update_id"], "msg": "field required", "type": "value_error.missing"}]},
        )
