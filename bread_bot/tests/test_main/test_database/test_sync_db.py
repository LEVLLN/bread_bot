...
class SyncDBTestCase(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_add_to_db(self):
        ...
        self.assertEqual(user.username, "some_user_1")
    ...
    async def test_rollbacks(self):
        user1 = User(username="rollback_user", email="rollback_user@mail.ru", hashed_password="password")
        user2 = User(username="rollback_user", email="rollback_user@mail.ru", hashed_password="password")
