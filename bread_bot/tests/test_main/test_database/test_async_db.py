...
class AsyncDBTestCase(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_rollbacks(self):
        ...
        with self.assertRaises(IntegrityError):
            await User.async_add_all(self.session, [user_object_2, ])
    ...
    async def test_bulk_update(self):
        ...
        async for row in result:
            ...
            row[0].username = row[0] \
                .username \
                .replace('bulk_update_', 'bulk_updated_')
            updated_users.append(row[0])
        await User.async_add_all(self.session, updated_users)
        result = await self.session.stream(
            select(User).where(
                User.username.in_([user.username for user in updated_users])
            )
        )
        async for row in result:
    ...
    async def test_filter(self):
        ...
        self.assertEqual(
            await User.async_offset_records(
                self.session,
                OffsetQueryParams(0, 1000)
            ),
            await User.async_all(self.session)
        )
