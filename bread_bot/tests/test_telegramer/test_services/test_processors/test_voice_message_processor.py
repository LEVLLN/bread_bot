...
class TestVoiceMessageProcessor:
    async def test_not_voice_message(self, db, edited_message_service):
        member_service = MemberService(db=db, message=edited_message_service.message)
        await member_service.process()
        processor = EditedMessageProcessor(message_service=edited_message_service, member_service=member_service)
    ...
    async def test_process_message(self, db, voice_message_service, property_factory,
        ...
        member_service = MemberService(message=voice_message_service.message, db=db)
        await member_service.process()
        chat = member_service.chat
        chat.is_voice_trigger = chat_is_voice_trigger
        await Chat.async_add(db=db, instance=chat)

        processor = VoiceMessageProcessor(message_service=voice_message_service, member_service=member_service)
