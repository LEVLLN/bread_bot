...
from bread_bot.utils.testing_tools import init_async_session, test_app, TEST_SERVER_URL
...
class CreateUserTestCase(unittest.IsolatedAsyncioTestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.schema = UserCreateSchema(**cls.data)

    async def test_create_user(self):
        async with AsyncClient(app=test_app, base_url=TEST_SERVER_URL) as ac:
            response = await ac.post("/users/create", data=json.dumps(self.data))
        user = await User.async_first(self.session, User.username == "test_create")
        ...
        self.assertDictEqual(response.json(), dict(user_schema))
    ...
    async def test_create_user_error(self, mock_add_schema: Mock):
        ...
        async with AsyncClient(app=test_app, base_url=TEST_SERVER_URL) as ac:
            response = await ac.post("/users/create", content=json.dumps(self.data))
        self.assertEqual(
            response.status_code,
            400,
        )
        self.assertEqual(response.json(), {"detail": "Ошибка регистрации пользователя"})
