...
from bread_bot.auth.methods.auth_methods import get_password_hash, create_access_token
...
class CreateTokenTestCase(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_get_token(self):
        ...
        user = await User.async_first(self.session, User.username == "create_token")
        self.assertIsNotNone(user)
        self.assertEqual(
            response.status_code,
            200,
        )
        self.assertDictEqual(response.json(), {"access_token": token, "token_type": "bearer"})
    ...
    async def test_get_token_user_not_found(self):
        ...
        self.assertIsNotNone(await User.async_first(self.session, User.username == "create_token"))
        self.assertEqual(
            response.status_code,
            401,
        )
