...
from bread_bot.main.settings import AUTO_MASK_LOGS, APP_NAME, \
    APP_VERSION, ENVIRONMENT, ENABLE_TELEMETRY
...
class JSONLogFormatter(logging.Formatter):
    ...
    def _format_log_object(record: logging.LogRecord) -> dict:
        ...
        span: Span = execution_context.get_current_span() \
            if ENABLE_TELEMETRY else EMPTY_VALUE

        if span:
            json_log_fields.trace_id = span.context_tracer.trace_id
            json_log_fields.span_id = span.span_id
            json_log_fields.parent_id = span.parent_span.span_id \
